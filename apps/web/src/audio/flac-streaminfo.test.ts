// QA #27: the rehearse export's STREAMINFO is finalized with the real
// total-samples count. Layout under test is the 42-byte bootstrap emitted
// by packages/codec codec_header(): fLaC + metadata block header +
// 34-byte STREAMINFO, total-samples = 36 bits at body offset 13.4.
...
describe("withTotalSamples (QA #27)", () => {
  it("patches only the 36-bit total-samples field", () => {
    ...
    // Input untouched (payload chunks reference the original bytes).
